...
class DataController extends Controller
{
    ...
    public function refreshStatusData_super($id){
        $device = Device::where('serial_number',$id)->with('latest_log:serial_number,log_dt,step,ec,tpv,alarm,pressure,created_at,updated_at','setpoints','latest_maintenance_critic_acid','latest_maintenance_pre_filter','latest_maintenance_post_filter','latest_maintenance_general_service','device_settings')->first();
        $today = date(Carbon::now());
        $thirtyOnedays = date(Carbon::now()->subDays(31));
        $previousDay = date(Carbon::now()->subDays(1));
        $volume = [];
    ...
    public function getDeviceAlarms($device_serial){
        ...
        $allAlarms = RawLogs::where('serial_number',$device_serial)->where('created_at','>=',Carbon::today()->subDays(631))
