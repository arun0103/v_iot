...
class DataController extends Controller
{
    ...
    public function refreshDashboardRows(){
        ...
        if($loggedInUser->role == "S"){
            $devices = Device::with('latest_log:serial_number,log_dt,ec','setpoints:device_id,pure_EC_target')->get();
        }else{
            $devices = Device::where('reseller_id',$loggedInUser->reseller_id)->with('latest_log:serial_number,log_dt,ec','setpoints:device_id,pure_EC_target')->get();
        }
        return response()->json($devices);
    ...
    public function refreshUserDashboardData(){
        ...
        $devices = Device::whereIn('id',$userDevicesIDs)->with('latest_log','setpoints','latest_maintenance_critic_acid','latest_maintenance_pre_filter','latest_maintenance_post_filter','latest_maintenance_general_service','device_settings')->get();
        // return response()->json($devices);
        $today = date(Carbon::now());
        $thirtyOnedays = date(Carbon::now()->subDays(31));
        $previousDay = date(Carbon::now()->subDays(1));
