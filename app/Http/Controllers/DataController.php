...
class DataController extends Controller
{
    ...
    public function refreshUserDashboardData(){
        ...
        $devices = Device::whereIn('id',$userDevicesIDs)->with('latest_log:serial_number,log_dt,created_at,ec,tpv','setpoints:device_id,pure_EC_target,volume_unit','latest_maintenance_critic_acid','latest_maintenance_pre_filter','latest_maintenance_post_filter','latest_maintenance_general_service','device_settings')->get();
        // return response()->json($devices);
        $today = date(Carbon::now());
        $thirtyOnedays = date(Carbon::now()->subDays(31));
        $previousDay = date(Carbon::now()->subDays(1));
